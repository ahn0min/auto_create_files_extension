...
function activate(context) {
    ...
    console.log('Congratulations, your extension "snowball-components-folder-template" is now active!');
        ...
        if (/components/g.test(event.path)) {
            ...
            vscode.workspace.fs.stat(eventURI).then(({ type }) => {
                ...
                if (type === vscode.FileType.Directory) {
                    ...
                    vscode.window.setStatusBarMessage("index.tsx 파일이 생성되었어요", 5000);
                    fs.writeFileSync(stylesFileURI, "// i'm styles");
                    vscode.window.setStatusBarMessage("styles.ts 파일이 생성되었어요", 5000);
                    return;
                }
            });
        }
